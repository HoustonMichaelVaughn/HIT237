...
from mango_pests.models import PestCheck, Pest
...
def summary_view(request):
    ...
    return render(request, "mango_pests/summary.html", {
...
def download_csv(request):
    ...
    writer.writerow(['Pest', 'Part of Plant', 'Confidence', 'Trees Checked', 'Positives', 'Date'])

    pest_checks = PestCheck.objects.all()
    for check in pest_checks:
        confidence = check.confidence_score if check.confidence_score is not None else 'N/A'
        writer.writerow([
            check.pest.name,
            check.part_of_plant,
            confidence,
